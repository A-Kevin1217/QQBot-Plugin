  ...
  async makeFriendMessage(data, event) {
    ...
    data.reply = async (msg) => {
      ...
      if (Array.isArray(msg) && config.btnTemplate && config.btnTemplate[data.self_id]) {
        ...
        msg.forEach((m, idx) => {
          if (m.type === 'button' || 
              (m.data && Array.isArray(m.data) && m.data.some(b => b.type === 'button'))) {
            btnIndices.push(idx)
          }
        })
        
        // 只有同时包含markdown和按钮时才处理
        if (hasMd && btnIndices.length > 0) {
          // 从后往前删除按钮
