...
async function rewriteMarkdownImageText(text, replaceUrl, replacements) {
  if (typeof text !== 'string' || !text.includes('![')) return { text, changed: 0 }
  ...
  for (const match of matches) {
    ...
    if (!replacements.has(url)) {
      replacements.set(url, Promise.resolve().then(() => replaceUrl(url)))
    }
  }

  const urls = await Promise.all(matches.map(match => replacements.get(match[1])))
  let result = ''
  let cursor = 0
  let changed = 0
  ...
  for (let index = 0; index < matches.length; index++) {
    ...
    const replacementUrl = String(urls[index] || '')
    result += text.slice(cursor, match.index)
    if (replacementUrl && replacementUrl !== sourceUrl) {
      result += match[0].replace(sourceUrl, replacementUrl)
      changed += 1
    } else {
      result += match[0]
    }
...
export async function rewriteMarkdownImageResources(messagePayload, replaceUrl) {
  if (typeof replaceUrl !== 'function') throw new Error('Markdown 图片重托管缺少 URL 处理函数')

  const markdown = messagePayload?.markdown
  ...
  if (typeof markdown.content === 'string') {
    const result = await rewriteMarkdownImageText(markdown.content, replaceUrl, replacements)
    markdown.content = result.text
    changed += result.changed
  }

  if (Array.isArray(markdown.params)) {
    for (const param of markdown.params) {
      if (!Array.isArray(param?.values)) continue
      for (let index = 0; index < param.values.length; index++) {
        const result = await rewriteMarkdownImageText(param.values[index], replaceUrl, replacements)
        param.values[index] = result.text
        changed += result.changed
      }
    }
  }
