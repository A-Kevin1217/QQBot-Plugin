...
| --- | --- | --- |
| 底层依赖 | `qq-official-bot@^1.2.3`（已兼容 GitHub `v1.2.4`） | `qq-official-bot@1.0.3` |
...
> GitHub latest `v1.2.4` 已带群消息事件区分和群成员权限字段；适配器同时补了加群申请事件映射兜底。如果 npm registry 暂未同步，`pnpm install` 仍会安装到已发布的 `1.2.3`，同步后会自动解析到 `1.2.4`。
...
23. `config/QQBot.yaml`中`imgBed`图床配置,当Bot上传图片失败时自动使用图床上传,支持多个图床回退,Redis缓存默认10分钟
    ...
    ```
    - 未配置的图床会自动跳过,无需删除
    - 图床回退顺序: CNB → B站 → 花瓣网 → COS → QQ频道 → Telegraph → 腾讯云CI备用 → 默认图片
    ...
    - `redis`
      ...
      - 缺点: 没有缓存所以有些没统计
24. 单聊和群聊富媒体已接入官方分片上传接口：分片会按平台下发配置自动重试，整轮失败默认重新申请上传任务 2 次；群聊 Markdown 默认开启图片转存结果校验，遇到 `40034004` 时默认退避重发 3 次，通常无需用户手动重发。
